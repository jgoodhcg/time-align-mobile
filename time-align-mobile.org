...
**** TODO Bug fixes 
     ...
     - [ ] Remove indication of long press
     - [ ] Drawer does not close on screen selection (needs an extra tap on the screen)
     - [ ] Copy over button needs an icon or other button indication
     - [ ] Pattern form does not display all elements
     - [ ] Pressing play and then cancel causes a validation error
     - [ ] Validation error on app init
     - [ ] Arrow to select next and previous are needed 
     - [ ] Duration label should be above value
     - [ ] Duration should indicate it is info only
     - [ ] Play button should be a stop button (not disabled)
     - [ ] Zoom buttons?
     ...
     - [ ] Text input fields need a fixed width
