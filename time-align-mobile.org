...
- [ ] Implement gesture handler experience on calendar page
  - [X] (zoom) Add top level pinch gesture that waits on scroll
  - [X] (scroll) Add scroll view that locks out on selection
  - [ ] (drag) Add pan gesture handler that waits for tap handler and locks out on no selection
  - [ ] (edit) Add tap handler that wraps element and waits on long press
  - [ ] (selection) Add long press wrapping element directly
  - [ ] Set up locking mechanism between long press and pan handlers
