...
(defn set-add-element-mode [db [_ state]]
  (assoc-in db [:add-element-mode] state))
...
(reg-event-db :set-add-element-mode [validate-spec persist-secure-store] set-add-element-mode)
